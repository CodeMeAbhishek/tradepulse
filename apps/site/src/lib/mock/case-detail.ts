...
const DETAILS: Record<string, Omit<CaseWorkbenchDetail, "caseId" | "checklist">> = {
  "case-inv-001": {
    uploadedFiles: [
      {
        name: "invoice_gulf_precision.pdf",
    ...
    invoiceFields: baseInvoiceFields().map((f) =>
      f.fieldPath === "seller.name"
        ? {
            ...f,
            value: "Gulf Precision Trading LLC",
            evidence: { page: 1, sourceText: "Seller: Gulf Precision Trading LLC" },
          }
        : f.fieldPath === "goods.quantity"
          ? {
              ...
              value: "100 MT",
              confidence: "HIGH",
              evidence: { page: 1, sourceText: "Qty: 100 MT" },
            }
          : f,
    ),
