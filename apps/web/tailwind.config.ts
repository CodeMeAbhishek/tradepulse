 ...
 *
 * Ramps are generated in oklch from those base colours: one hue per family,
 * lightness following Tailwind's 50..950 convention, chroma tapered at the ends
 * because very light and very dark colours cannot hold mid-step saturation.
 * The saved colour sits exactly on its mid step.
 */
...
const config: Config = {
  ...
  theme: {
    extend: {
      /**
       * Three families, matching the design system. Sizes are deliberately left
       * on Tailwind's default scale: the old 9-step scale runs much larger at
       * the top (h1 46px vs text-3xl 30px) and would reflow a dense, table-heavy
       * workbench it was never designed for.
       */
      fontFamily: {
      ...
      colors: {
        // Named tokens, for anything written from here on.
        ...
        // Existing class names, repointed at the palette. The design system has
        // no blue or teal, so those families resolve to ink neutrals.
        slate: neutral,
        gray: neutral,
        zinc: neutral,
        neutral: neutral,
