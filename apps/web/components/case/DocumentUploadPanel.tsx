...
export function DocumentUploadPanel({
  ...
  return (
    <section className="rounded border border-slate-700/80 bg-slate-950/40 p-4" aria-labelledby="upload-heading">
      ...
      <label className="mb-4 block text-sm text-slate-300">
        Transaction profile
        <select
          className="mt-1 w-full max-w-md rounded border border-slate-600 bg-slate-900 px-3 py-2 text-slate-100"
          value={profile}
      ...
      <div className="mb-4">
        <label className="block text-sm text-slate-300">
          ...
          <input
            type="file"
            multiple
            className="mt-1 block w-full text-sm text-slate-400 file:mr-3 file:rounded file:border-0 file:bg-slate-800 file:px-3 file:py-1.5 file:text-slate-100"
            onChange={(e) => {
        ...
        <ul className="mt-2 space-y-1 text-sm text-slate-300">
          ...
          {files.length === 0 ? (
            <li className="text-slate-500">No files listed yet.</li>
          ) : null}
        </ul>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {checklist.map((item) => (
              <tr key={item.documentType} className="border-t border-slate-800 align-top">
                <td className="py-2.5 pr-3 text-slate-200">{item.label}</td>
                <td className="py-2.5 pr-3 font-mono text-xs text-sky-200">
                  {documentStateLabel(item.state)}
                </td>
                <td className="py-2.5 pr-3 text-slate-300">
                  {item.blocker ? "Yes — required for this profile" : "No"}
                </td>
                <td className="py-2.5 text-slate-400">{item.reason}</td>
              </tr>
            ))}
          </tbody>
        </table>
