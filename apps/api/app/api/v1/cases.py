...
from app.deps import PlatformStateDep, require_database_ready
...
def create_case_endpoint(body: CreateCaseRequest, platform: PlatformStateDep) -> CaseRecord:
    case = create_case(
...
def list_cases_endpoint(platform: PlatformStateDep) -> list[CaseSummary]:
    return [to_case_summary(c) for c in platform.cases.list_cases()]


@router.get("/cases/{case_id}", response_model=CaseRecord)
def get_case_endpoint(case_id: str, platform: PlatformStateDep) -> CaseRecord:
    return to_case_record(platform.cases.require(case_id))
...
async def upload_document_endpoint(
    ...
    platform: PlatformStateDep = None,
) -> dict:
...
def case_action_endpoint(case_id: str, body: CaseActionRequest, platform: PlatformStateDep) -> CaseRecord:
...
def case_audit_endpoint(case_id: str, platform: PlatformStateDep) -> list[dict]:
    platform.cases.require(case_id)
    events = platform.audit.for_case(case_id)
    return [e.model_dump(mode="json") for e in events]


@router.get("/cases/{case_id}/versions")
def case_versions_endpoint(case_id: str, platform: PlatformStateDep) -> list[dict]:
...
def case_replay_endpoint(case_id: str, body: ReplayRequest, platform: PlatformStateDep) -> dict:
    case = platform.cases.require(case_id)
...
def case_policy_endpoint(case_id: str, platform: PlatformStateDep) -> dict:
    return evaluate_case_policy(case_id, state=platform).model_dump(mode="json")


@router.get("/cases/{case_id}/identity-ladder", response_model=list[IdentityLadderView])
def case_identity_ladder_endpoint(case_id: str, platform: PlatformStateDep) -> list[dict]:
...
def case_examiner_pack_endpoint(case_id: str, platform: PlatformStateDep) -> ExaminerCasePack:
